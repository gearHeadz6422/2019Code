...
void GripPipeline::Process(cv::Mat& source0) {
	...
	//double hslThresholdHue[] = { 0.0, 77.09897610921502 };
	double hslThresholdHue[] = { 0.0, 120.0 };
	//double hslThresholdSaturation[] = { 34.39748201438849, 218.01194539249144 };
	double hslThresholdSaturation[] = { 25.0, 255.0 };
	//double hslThresholdLuminance[] = { 167.4010791366906, 255.0 };
	double hslThresholdLuminance[] = { 150.0, 255.0 };
	hslThreshold(hslThresholdInput, hslThresholdHue, hslThresholdSaturation,
			hslThresholdLuminance, this->hslThresholdOutput);
	//Step Find_Contours0:
	//input
	...
	double filterContoursMinWidth = 45.0;  // default Double
...
void GripPipeline::hslThreshold(cv::Mat &input, double hue[], double sat[],
		double lum[], cv::Mat &out) {
	cv::cvtColor(input, out, cv::COLOR_BGR2HLS);
	cv::inRange(out, cv::Scalar(hue[0], lum[0], sat[0]),
			cv::Scalar(hue[1], lum[1], sat[1]), out);
}
...
void GripPipeline::filterContours(
	...
	for (std::vector<cv::Point> contour : inputContours) {
		...
		//if (cv::contourArea(contour, true) == cv::contourArea(contour, true)) {
		//		visionPwr = 0.3;
		//	}
	}
}
